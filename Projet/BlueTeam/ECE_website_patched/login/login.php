...
function checkLoginAttempts($ip_address) {
    ...
    // Nettoyer les anciennes tentatives (plus de 2 heures)
    $cleanup = $pdo->prepare("DELETE FROM login_attempts WHERE attempt_time < NOW() - INTERVAL 2 HOUR");
    $cleanup->execute();
    
    // Vérifier le nombre de tentatives récentes
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM login_attempts WHERE ip_address = :ip AND attempt_time > NOW() - INTERVAL 15 MINUTE");
...
function recordLoginAttempt($ip_address, $username, $success = 0) {
    global $pdo;
    
    $stmt = $pdo->prepare("INSERT INTO login_attempts (ip_address, username, success, attempt_time) VALUES (:ip, :username, :success, NOW())");
    $stmt->execute([
        'ip' => $ip_address,
        'username' => $username,
        'success' => $success
...
// Obtenir l'adresse IP réelle derrière un proxy si applicable
...
$attempts = checkLoginAttempts($ip_address);
if ($attempts >= $max_attempts) {
    $is_locked = true;
    $wait_time = 15; // minutes
    $error = "Trop de tentatives de connexion. Veuillez réessayer dans {$wait_time} minutes.";
}

if (isset($_POST['submit']) && !$is_locked) {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($user && (md5($password) === $user['password'])) {
            // Connexion réussie
            recordLoginAttempt($ip_address, $username, 1); // Enregistrer la tentative réussie
            ...
            // Echec de connexion - enregistrer la tentative échouée
            recordLoginAttempt($ip_address, $username, 0);
